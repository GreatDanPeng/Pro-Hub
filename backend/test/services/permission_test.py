...
from .role_data import ambassador_role, acm_leader_role, cssg_leader_role, root_role
from .user_data import root, ambassador, user, leader, president, student
...
def test_permission_update_cssg_organization(permission_svc: PermissionService):
    """Tests that leader can update cssg organization"""
    assert (
        permission_svc.check(leader, "organization.update", "organization/cssg") is True
    )


def test_permission_update_acm_organization(permission_svc: PermissionService):
    """Tests that president can update acm organization"""
    assert (
        permission_svc.check(president, "organization.update", "organization/acm")
        is True
    )


def test_no_permission_update_cssg_organization(permission_svc: PermissionService):
    """Tests that user and president of ACM cannot update cssg organization"""
    assert (
        permission_svc.check(user, "organization.update", "organization/cssg") is False
    )
    assert (
        permission_svc.check(president, "organization.update", "organization/cssg")
        is False
    )


def test_no_permission_update_acm_organization(permission_svc: PermissionService):
    """Tests that user and president of ACM cannot update cssg organization"""
    assert (
        permission_svc.check(leader, "organization.update", "organization/acm") is False
    )


def test_no_update_organization_permission(permission_svc: PermissionService):
    """Tests that a member cannot update organization"""
    assert permission_svc.check(user, "organization.update", "organization") is False


def test_member_get_all_users_cssg(permission_svc: PermissionService):
    """Tests that a member can get all users in cssg"""
    assert (
        permission_svc.check(user, "organization.get_all_users", "organization/cssg")
        is True
    )


def test_guest_cannot_get_all_users_cssg(permission_svc: PermissionService):
    """Tests that a guest cannot get all users in cssg"""
    assert (
...
def test_revoke_role_permission(permission_svc: PermissionService):
    ...
    permission_svc.revoke(root, ambassador_permission[0])
